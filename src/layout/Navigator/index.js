...
import { APIs, useAsyncAxios } from '@/utils/axios';
import './style.scss';
...
function Navigator() {
	...
	const [test, refetchMenu] = useAsyncAxios(APIs.COMMON.MENU, {}, [], false);
	const { loading: isLoadingMenu, data: menuList, error: loadingMenuError } = test;
	// refetch()
	...
	}, [pathname, isLoadingMenu, menuList])

	if(isLoadingMenu) return (<div>메뉴 로딩 중</div>)	 // Loading 관련 rendering도 만들어 보자....!
	if(loadingMenuError) return <div>메뉴 로딩 중 에러 발생</div>
...
function Menu( { item, activeIndex, onSelect } ) {
	...
	return (
		...
		>
			{
				item.link 
				? <NavLink to={item.link} data-id={item.id}> {item.name} </NavLink> 
